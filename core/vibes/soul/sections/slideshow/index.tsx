...
export function Slideshow({ slides, playOnInit = true, interval = 5000, className }: Props) {
  ...
  return (
    <section
      className={clsx(
        'relative h-[80vh] bg-[var(--slideshow-background,color-mix(in_oklab,hsl(var(--primary)),black_0%))] @container',
        className,
      )}
    >
            ...
            ({ title, description, showDescription = true, image, cta, showCta = true }, idx) => {
              return (
                <div
                  ...
                  key={idx}
                >
                  <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-[var(--slideshow-mask,hsl(var(--foreground)/0%))] to-transparent">
